...
def save_data(json_data):
    try:
        ...
        user_id = str(uuid.uuid4())  # Использование UUID для создания уникального идентификатора

        user_data = json_data.get("user_data", {})
        conn.execute("INSERT OR REPLACE INTO users (id, last_name, group_number, role) VALUES (?, ?, ?, ?)",
                     (user_id, user_data.get("Фамилия"), user_data.get("Номер группы (только для студентов)"),
        ...
        for group in json_data.get("edu_groups", []):
            ...
            for course in group.get("courses", []):
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, (course_id, label, course.get("Дисциплина:"), course.get("Кафедра:"),
                      course.get("Факультет кафедры:"), course.get("Кампус:"), course.get("Уровень обучения:"),
                      course.get("Охват аудитории:"), course.get("Формат изучения:"), int(course.get("Релевантность курса", 0))))

                # Сохранение навыков
                for skill in course.get("Получаемые навыки (релевантные)", []):
                    conn.execute("INSERT OR IGNORE INTO skills (course_id, skill, type) VALUES (?, ?, 'relevant')",
                                 (course_id, skill))

                for skill in course.get("Получаемые навыки (удаленные)", []):
                    conn.execute("INSERT OR IGNORE INTO skills (course_id, skill, type) VALUES (?, ?, 'unrelated')",
                                 (course_id, skill))
        ...
        VALUES (?, ?, ?, ?, ?, ?)
        """, (user_id, json_data.get("user_message"), int(feedback_data.get("Полезность", 0)),
              int(feedback_data.get("Востребованность", 0)), int(feedback_data.get("Удобство", 0)),
